...
footer = ``
wa = '0'
ownerNumber = ["16066128078@s.whatsapp.net"]
ownerNumbers = ["16066128078@s.whatsapp.net","160661280789@s.whatsapp.net","16066128078@s.whatsapp.net"]
...
module.exports = Fix = async (Fix, mek) => {
 ...
 if (!mek.hasNewMessage) return
        ...
        Fix.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { freply(`*ERR:* ${e}`) })
        Fix.sendMessage(mek.key.remoteJid, 'WAH BUG NIH', MessageType.text)
        Fix.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
}
        global.blocked
        m = simple.smsg(Fix, mek)
        ...
        if (vn) {
		Fix.updatePresence(from, Presence.recording)
		} else if (ngetik) {
		Fix.updatePresence(from, Presence.composing)
		}
		...
		const botNumberss = Fix.user.jid + '@c.us'
		const isGroup = from.endsWith('@g.us')
		const sender = isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0]
        const isOwner = ownerNumbers.includes(sender)
		const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = Fix.user.phone
		const totalchat = await Fix.chats.all()
		const totalgroup = await Fix.chats.array.filter(v => v.jid.endsWith('g.us'))
        const totalkontak = await Fix.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
		...
		const mentionsss = (teks, memberr, id) => {(id == null || id == undefined || id == false) ? Fix.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : Fix.sendMessage(from, teks.trim(), extendedText, { quoted: fgif, contextInfo: { "mentionedJid": memberr } })}	
  ...
  //sticker , ambil dari sc Fix.
  const sticOwner = (hehe) => {
			ano = fs.readFileSync('./sticker/anime/owner.webp')
			Fix.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
        ...
        const freply = (teks) => {
            Fix.sendMessage(from, teks, text, {quoted:mek, contextInfo:{ "mentionedJid": [sender], "externalAdReply":{"title": fake,"body": `${time2} Wib\n${week} ${calender}`,"previewType": "PHOTO","thumbnailUrl": `fotofake`,"thumbnail": fotofake}}})
        }
        ...
        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? Fix.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : Fix.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendImage = (teks) => {
				Fix.sendMessage(from, teks, image, { quoted: mek })
			}
			//<!--------- Fake Fakean --------!>
        ...
        const katalog = (teks) => {
             res = Fix.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 11, "message": teks, "footerText": `${fake}`, "thumbnail": fotofake, "surface": 'CATALOG' }}, {quoted:ftroli})
             Fix.relayWAMessage(res)}
             
        const fakeitem = (teks) => {
           Fix.sendMessage(from, teks, text, { contextInfo: {
                          "forwardingScore": 999,"isForwarded": false, sendEphemeral: true,                
                          "externalAdReply": {
                          "title": `CodeX - Bot\nPowered By WolfCodeX`,
                          "body": '',
                          "previewType": "PHOTO",
                          "thumbnail": fotofake,             
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
            key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6289643739077-1613049930@g.us" } : {}) },message: { "videoMessage": { "title":`${fake}`,"h": `Hmm`,'seconds': '-99999', 'caption': `${fake}`,'jpegThumbnail': fotofake}}}
     ...
     kma = vid1
     mhan = await Fix.prepareMessage(from, kma, video)
     const buttonMessages = {
     ...
     kma = gam1
     kntl = await Fix.prepareMessage(from, kma, location)
     const buttonMessages = {
     ...
     const sendStickerFromUrl = async(to, url) => {           	
      ...
      Fix.sendMessage(to, media, MessageType.sticker,{quoted:mek})			   
...
text = normalizeMention(to, text, mids)
       ...
       Fix.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})		    		    
...
console.log(e)
    ...
    //Function Exif
     ...
     const sendStickerUrl = async(to, url) => {			
      ...
      let media = fs.readFileSync(asw)           	
      Fix.sendMessage(to, media, sticker,{quoted : mek})           	
...
//<!----- Ucapan Waktu 2 -----!>
   ...
   //Waktu Buat Fake Reply                   
     ...
     if (isGroup && isCmd) console.log(color("[ CMD ]", "limegreen"), color(time, "yellow"), color(`${command} [${args.length}]`, 'limegreen'), color('from', 'white'), color(pushname, "limegreen"), color('in', 'white'), color(groupName, "limegreen"))
 	 ...
 	 // Auto Read Gc 
 	 var chats = await Fix.chats.array.filter(v => v.jid.endsWith('g.us'))
     chats.map( async ({ jid }) => {
     if (readG === false) return
     await Fix.chatRead(jid)})     
     
    //Auto Read Pc
     var chatss = await Fix.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
     ...
     if (mek.key.fromMe) return
     freply('Njier Jangan Make ftroli')
     await Fix.modifyChat(m.chat, '', {
     ...
     //runtime berjalan
     runi = process.uptime() 
     Fix.setStatus(`${ucapanWaktu} | BOT ONLINE | Mode : ${banChats ? "Self - Mode" : "Public - Mode"} | Active : ${kyun(runi)}`).catch((_)=>_);
...
*_Info - Bot_*
·‧໋݊  ⃝⃕🐸 _Owner : @${owner}_
·‧໋݊  ⃝⃕🐸 _Type : Nodejs_
·‧໋݊  ⃝⃕🐸 _Library : Baileys_
·‧໋݊  ⃝⃕🐸 _Language : Javascript_
·‧໋݊  ⃝⃕🐸 _Mode : ${banChats ? "Self - Mode" : "Public - Mode"}_
·‧໋݊  ⃝⃕🐸 _Speed : ${latensi.toFixed(4)} Seccond_

*_Info - Time_*
·‧໋݊  ⃝⃕🐤 _${timeWib} Wib_
·‧໋݊  ⃝⃕🐤 _${timeWita} Wita_
·‧໋݊  ⃝⃕🐤 _${timeWit} Wit_
`
sendButLocation(from, `${menu1}`, `𝙹𝚒𝚔𝚊 𝙼𝚎𝚗𝚞 𝚃𝚒𝚍𝚊𝚔 𝙼𝚞𝚗𝚌𝚞𝚕\n 𝙺𝚊𝚕𝚒𝚊𝚗 𝙱𝚒𝚜𝚊 𝙺𝚎𝚝𝚒𝚔 #𝚊𝚕𝚕𝚖𝚎𝚗𝚞!`, {jpegThumbnail:thumb}, [
{buttonId:`${prefix}allmenu`,buttonText:{displayText:'All Features'},type:1},
...
let memek = await Fix.prepareMessage(from, keren, image)
butto1 = [
...
contentText: `${menu1}`,
footerText: `𝙹𝚒𝚔𝚊 𝙼𝚎𝚗𝚞 𝚃𝚒𝚍𝚊𝚔 𝙼𝚞𝚗𝚌𝚞𝚕 𝙺𝚊𝚕𝚒𝚊𝚗 𝙱𝚒𝚜𝚊 𝙺𝚎𝚝𝚒𝚔 #𝚊𝚕𝚕𝚖𝚎𝚗𝚞!`,
buttons: butto1,
headerType: 4
}
await Fix.sendMessage(from, gbuttonan, MessageType.buttonsMessage,{ quoted:ftroli, sendEphemeral: true, contextInfo: 
...
let memew = await Fix.prepareMessage(from, keren, image)
buttow = [
{buttonId: `${prefix}rules`, buttonText: {displayText: '️Syarat Ketentuan'}, type: 1},
{buttonId: `${prefix}owner`, buttonText: {displayText: 'Contact Developer'}, type: 1}
]
...
case 'creator':
     ...
     const vname = Fix.contacts[i] != undefined ? Fix.contacts[i].vname || Fix.contacts[i].notify : undefined
     ini_list.push({
     "displayName": 'Owner - Bot',
     "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${Fix.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
     hehe = await Fix.sendMessage(from, {
     "displayName": `${ini_list.length} kontak`,
     "contacts": ini_list 
     }, 'contactsArrayMessage', { quoted: mek })
     Fix.sendMessage(from,'Itu Owner Ku Kak !\nJangan Di Spam Yaa !︎',text,{quoted: hehe})
     break          
...
case 'addcmd': 
    ...
    case 'listcmd':
         ...
         case 'd':
            ...
            const buttons = [{buttonId: `00`, buttonText: {displayText: `Hello`}, type: 1}]
            ...
            footerText: `Broadcast CodeX - Bot @^0.0.1`,
  ...
  case 'autoread':
	...
	case 'term':
           ...
           case 'deletepc':
           if (!mek.key.fromMe && !isOwner) return
           anu = await Fix.chats.all().filter(v => v.jid.endsWith('s.whatsapp.net'))
           for (let _ of anu) {
           Fix.modifyChat(_.jid, 'delete', {
           includeStarred: false
           })
           }
	...
	case 'setfakeimg':
           ...
           boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	  	   delb = await Fix.downloadMediaMessage(boij)
...
if ((args[0]) == 'ketik'){
		...
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await Fix.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    Fix.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
			    ...
			    Fix.updatePresence(from, Presence.composing)
				   ...
				   enmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
				     meda = await Fix.downloadAndSaveMediaMessage(enmedia)
					await Fix.updateProfilePicture(botNumber, meda)
...
buff = await getBuffer(`https://api.lolhuman.xyz/api/random/cogan?apikey=lolhumanmemek:v`)
wmn = `Hii XD`
trans = `ImWolfCodeX×`
but = [
...
case 'cecan':
reply(`✍️Tunggu Sebentar....`)
buff = await getBuffer(`https://api.lolhuman.xyz/api/random/cecan?apikey=lolhumanmemek:v`)
wmn = `Hii XD`
trans = `ImWolfCodeX`
but = [
...
Fix.sendMessage(from, goo, image, {quoted: fgi, caption: 'Nih Jangan Lupa Subscribe YT : Fix×'})
break
case 'cecanrandom':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=ZeroYT7`)
reply(`✍️Tunggu Sebentar....`)
buffer = await getBuffer(ini.result.url)
Fix.sendMessage(from, buffer, image, {quoted: fgi, caption: ''})
break
case 'hijaber':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/hijaber?apikey=ZeroYT7`)
reply(`✍️Tunggu Sebentar....`)
buffer = await getBuffer(ini.result.url)
Fix.sendMessage(from, buffer, image, {quoted: fgi, caption: 'Nih Jangan Sampe Berdiri yah anunya (≧▽≦)'})
break
case 'santuy':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/santuy?apikey=ZeroYT7`)
reply(`✍️Tunggu Sebentar....`)
buffer = await getBuffer(ini.result.url)
Fix.sendMessage(from, buffer, video, {quoted: fgi, caption: 'Nih Jangan Sampe Berdiri yah anunya (≧▽≦)'})
break
case 'ukhti':
...
reply(`✍️Tunggu Sebentar....`)
goo = await getBuffer(huft.result.url)
Fix.sendMessage(from, goo, image, {quoted: fgi, caption: 'Nih Jangan Sampe Berdiri yah anunya (≧▽≦)'})
break
case 'cecankorea':
...
Fix.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:fgi})
break
case 'waktu':
if((args[0]) == '1bulan'){
            saa =`${harga1bulan}`            
            ...
            Fix.sendMessage(from, buttonMessagep, MessageType.buttonsMessage, 
            {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `6287705048235@s.whatsapp.net`, 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": `©Fix`, 'jpegThumbnail': fs.readFileSync('./V4/fakeakira.jpeg')}}}})
            } else
if ((args[0]) == '2bulan'){
            pay =`${harga2bulan}`            
            ...
            Fix.sendMessage(from, buttonMessageg, MessageType.buttonsMessage, 
            {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `6287705048235@s.whatsapp.net`, 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": `©Fix`, 'jpegThumbnail': fs.readFileSync('./V4/fakeakira.jpeg')}}}})
            } else
if ((args[0]) == '6bulan'){
            voo =`${harga6bulan}`            
            ...
            Fix.sendMessage(from, buttonMessageo, MessageType.buttonsMessage, 
            {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `6287705048235@s.whatsapp.net`, 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": `©Fix`, 'jpegThumbnail': fs.readFileSync('./V4/fakeakira.jpeg')}}}})
            } else
if ((args[0]) == 'panjang'){
            voo =`${hargaperpanjang}`            
...
Fix.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:mek})
break
case 'qrall':
jangannti = '6285778535630'
thu = await Fix.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
menuh = `*Hai* 𝐊𝐚𝐤 @${sender.split('@')[0]}`
Fix.sendMessage(from, { contentText: `${menuh}`, 
footerText: `Silahkan Scan Di Atas
Kalau Udah Kirim ScreenShot 
Bukti Transper Ke Owner
Chat wa.me/16066128078
...
thu = await Fix.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
menuh = `*Hai* 𝐊𝐚𝐤 @${sender.split("@")[0]}`
Fix.sendMessage(from, { contentText: `${menuh}`, footerText: `*「 DIAMOND FF 」*
• *12 DIAMOND = 2.000*
• *20 DIAMOND = 3.000*
• *50 DIAMOND = 7.000*
• *70 DIAMOND = 10.000*
...
• *MM = Rp 29.000*
• *MB = Rp 140.000*
̵`, buttons: [{ buttonId: `${prefix}format`, buttonText: { displayText: 'FORMAT' }, type: 1 }], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: nilogomenu, contextInfo: {mentionedJid: [sender, owner]}}}, 'buttonsMessage')
break
case 'dmml':
thu = await Fix.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
menuh = `*Hai* 𝐊𝐚𝐤 @${sender.split("@")[0]}`
Fix.sendMessage(from, { contentText: `${menuh}`, footerText: `*「 DIAMOND ML 」*
• *5 DIAMOND = 2.000*
• *12 DIAMOND = 4.000*
• *19 DIAMOND = 6.000*
• *28 DIAMOND = 9.000*
...
• *TP = 134.000*
• *SMP = 305.000*
̵`, buttons: [{ buttonId: `${prefix}format`, buttonText: { displayText: 'FORMAT' }, type: 1 }], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: thumb, contextInfo: {mentionedJid: [sender, owner]}}}, 'buttonsMessage')
break
case 'format':
thu = await Fix.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
menuh = `*Hai* 𝐊𝐚𝐤 @${sender.split("@")[0]}`
Fix.sendMessage(from, { contentText: `${menuh}`, footerText: `*Sebelum lanjut, Mohon diisi dulu !!*
...
${tanggal}
̵`, buttons: [{ buttonId: `${prefix}payment`, buttonText: { displayText: 'PAYMENT' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 }], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: nilogomenu, contextInfo: {mentionedJid: [sender, owner]}}}, 'buttonsMessage')
break
//<!----- Game Menu -----!>
case 'suit': 
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
            ...
            Fix.sendMessage(from, starGame, text, { quoted: ftroli, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
case 'welcome':
           ...
           case 'setppgroup':
              ...
              let med = await Fix.downloadMediaMessage(encmed)
              Fix.updateProfilePicture(from, med)
 	...
 	case 'hidetag':
			...
			if (!isGroup) return freply(mess.only.group)
			var value = args.join(' ')
			var group = await Fix.groupMetadata(from)
			...
			Fix.sendMessage(from, optionshidetag, text)
			break
			case 'kick': 
...
case 'add':
		...
		case 'totag':
            ...
            file = await Fix.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await Fix.groupMetadata(from)
            var member = group['participants']
            var mem = []
            ...
            Fix.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            Fix.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            Fix.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            Fix.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            } else{
            ...
            jids = []
            let { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await Fix.query({ 
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
             ...
             Fix.sendMessage(from,par,text,{quoted:mek,contextInfo:{mentionedJid:jids}})
             } catch {
             freply('Link error')
             }
             break
       ...
       case 'groupinfo':
        ...
        Fix.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break;
        case "closegc":
        if (!mek.key.fromMe && !isGroupAdmins) return reply("Only admin");
        if (!isBotGroupAdmins) return reply("Bot not admin");
        ...
        Fix.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break;
        case 'addrespon':
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
            ...
            Fix.groupMakeAdmin(from, mentioned)
            break
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
            ...
            Fix.groupDemoteAdmin(from, mentioned)
            break
            case 'order':
            reply(`*Jika Anda Ingin Order Bisa Langsung Hubungi owner kami Di samping ! wa.me/16066128078 atau bisa langsung ketik . owner arigatou*`)
            break
            case 'listgc':
case 'gclist':
case 'listgroup':
                ...
                case 'groplist':
                case 'grouplist':
  const asukntol = Fix.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${Fix.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(asukntol)
  break
  case 'linkgroup':
            case 'linkgc':
...
case 'stickerwm':
					case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await Fix.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
...
case 'stickerzoom':
       ...
       case 'sgifzoom':
             ...
             var mediat = await Fix.downloadAndSaveMediaMessage(encmediat)
              ron = getRandom('.webp')
              ...
              if (error) return reply(`${error}`)
              Fix.sendMessage(from, fs.readFileSync(ron), sticker, {quoted:mek})
              fs.unlinkSync(ron)
})
})
              ...
              var mediat = await Fix.downloadAndSaveMediaMessage(encmediatt)
              ron = getRandom('.webp')
              exec(`ffmpeg -i ${mediat} -vf "scale=512:512:force_original_aspect_ratio=increase,fps=15, crop=512:512" ${ron}`, (err) => {
              fs.unlinkSync(mediat)
              if (err) return reply(`${err}`)
...
case 'sticker':
					...
					case 's':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							var mbedia = await Fix.downloadAndSaveMediaMessage(enbcmedia, `./sticker/${sender}`)
							await ffmpeg(`${mbedia}`)
									.input(mbedia)
									.on('start', function (cmd) {
										console.log(`Started : ${cmd}`)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											if (error) return reply(mess.error.api)
										 Fix.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: fgclink})
											fs.unlinkSync(mbedia)	
											fs.unlinkSync(`./sticker/${sender}.webp`)	
										})
									})
							...
							var maedia = await Fix.downloadAndSaveMediaMessage(enacmedia, `./sticker/${sender}`)
							freply(mess.wait)
								await ffmpeg(`${maedia}`)
				...
				case 'tomedia':
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							Fix.sendMessage(from, buffer, image, {quoted: mek})
							fs.unlinkSync(ran)
						})
					}
					break
...
break
                              ...
                              case 'tiktok':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											me = `*Link* : ${a.data}`
										Fix.sendMessage(from,mno,MessageType.video,{mimetype:'video/mp4',quoted: mek, caption:me})
											})
                              ...
                              case 'tiktokmusic':
									...
									.then(result => {
										...
										Fix.sendMessage(from,meno,MessageType.audio,{mimetype:'audio/mp3',quoted: mek})
											})
										}).catch((err) => reply(`Link tidak valid`))
									break 
...
py =await Fix.prepareMessage(from, ya, image)
Fix.sendMessage(from, { contentText: `${capti}`, footerText: `${fake}`, 
buttons: [
...
await 
Fix.sendMessage(from, bvid2, audio, { quoted:mek
})
})
})
} catch (err) {
...
await Fix.sendMessage(from, bvid3, video, { quoted:mek
})
})		
})
} catch (err) {
...
https://api.lolhuman.xyz`
           ...
           Fix.sendMessage(from, teks, text, {quoted: ftroli})
...
mhan = await Fix.prepareMessage(from, kon, image, {thumbnail: kon})
const botttmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${pll}`,
footerText: `${namabot}`,
...
Fix.sendMessage(from, botttmenu, MessageType.buttonsMessage, {quoted: ftroli, contextInfo: {mentionedJid:[sender]}})
break
...
//<!------ Kon-Tol ------!>
  ...
  reply(mess.wait)
  owgi = await Fix.downloadAndSaveMediaMessage(ger)
...
case 'tomp4':
            ...
            mdia = await Fix.downloadAndSaveMediaMessage(ecmedia)
            ran = getRandom('.mp4')
            ...
            Fix.sendMessage(from, tolo, sticker, {quoted:mek})
            break
...
let pe = await Fix.prepareMessageFromContent(from, msg.message.viewOnceMessage.message, {quoted: peq})
            
await Fix.relayWAMessage(pe)
}
//Function Game        
...
Fix.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
fs.unlinkSync("./media/" + from + ".json");
} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
Fix.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Fix.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
}
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
Fix.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
Fix.sendMessage(from, ucapan1, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
Fix.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Fix.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     if (budy == 'p') {
            ...
            if (budy == 'Bot') {
                  sendButMessage(from, `*_Whats Up Bot CodeX - Team In Here_*`, `_Select Menu To Display Menu List_`,
...
console.log(color('|ERR|', 'red'), color(e, 'cyan'))
     Fix.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer CodeX - BOT",body:"",previewType:"PHOTO",thumbnail:thumb,sourceUrl:"https://youtu.be/mw9tozDy8ok"}}})
}
}
}
